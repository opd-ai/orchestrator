...
2. [x] Persist rejected patches under `logs/rejected_patches/` with the rejection reason in the structured log stream.
3. [x] Generate `AUTONOMOUS_RUN_SUMMARY.md` at the end of each run with task counts, duration, branch, and blocked-task reasons.
4. [x] Emit periodic summary metrics for completed tasks, blocked tasks, retries, and applied patches.
5. [x] Add cleanup and retention rules so observability artifacts do not grow without bound.
